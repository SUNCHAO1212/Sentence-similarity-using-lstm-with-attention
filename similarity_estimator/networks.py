...
class LSTMEncoder(nn.Module):
    ...
    def __init__(self, vocab_size, opt, is_train=False):
        ...
        self.lstm_rnn = nn.LSTM(input_size=self.opt.embedding_dims, hidden_size=self.opt.hidden_dims, num_layers=1, bidirectional=True)

        """"""
        # bidirectional lstm
        self.weight_W_word = nn.Parameter(torch.FloatTensor(self.opt.hidden_dims * 2, self.opt.hidden_dims * 2))
        self.bias_word = nn.Parameter(torch.FloatTensor(self.opt.hidden_dims * 2, 1))
        self.weight_proj_word = nn.Parameter(torch.FloatTensor(self.opt.hidden_dims * 2, 1))
        # not bidirectional
    ...
    def initialize_hidden_plus_cell(self, batch_size):
        ...
        # bidirectional lstm
        zero_hidden = Variable(torch.randn(1 * 2, batch_size, self.opt.hidden_dims))
        zero_cell = Variable(torch.randn(1 * 2, batch_size, self.opt.hidden_dims))
        # not bidirectional
